#include "/usr/csshare/pkgs/glew-1.13.0/include/GL/glew.h"
...
GLuint VerticleShaderInt;
GLuint FragmentShaderInt;
GLuint ProgramInt;

GLuint loadShader(const char* vertexfilename, const char* fragmentfilename)
{ 
	string temp = getTextFile(vertexfilename);
	const char* vertexfile = temp.c_str();
	int T = 0;
    while(vertexfile[T] != 0)
    {
		T++;
	}
    ...
    // glCreateShader
    VerticleShaderInt = glCreateShader(GL_VERTEX_SHADER);
    FragmentShaderInt = glCreateShader(GL_FRAGMENT_SHADER);
    // glShaderSource
    glShaderSource(VerticleShaderInt, 1, &vertexfile, NULL );
	T = 0;
    temp = getTextFile(fragmentfilename);
	const char* fragmentfile = temp.c_str();
    while(fragmentfile[T] != 0)
    {
		T++;
	}
    
	glShaderSource(FragmentShaderInt, 1, &fragmentfile, NULL );
    // glCompileShader
    glCompileShader(VerticleShaderInt);
    glCompileShader(FragmentShaderInt);

	// glCreateProgram
    ProgramInt = glCreateProgram();

    // glAttachShader
    glAttachShader(ProgramInt, VerticleShaderInt);
    glAttachShader(ProgramInt, FragmentShaderInt);
       
    // glLinkProgram
    glLinkProgram(ProgramInt);
    glUseProgram(ProgramInt);
	...
	debugShader(VerticleShaderInt,FragmentShaderInt,ProgramInt);
	return ProgramInt;
}
